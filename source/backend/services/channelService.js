...
const channelService = {
  ...
  updateChannel: (oldUrl, newUrl, newName) => {
    const data = readChannels();
    let changed = false;
    
    // If URL is changing, check for duplicates first
    if (oldUrl !== newUrl) {
      for (const key in data) {
        if (key === 'everyone') continue;
        if (data[key].some(c => c.url === newUrl)) {
          throw new DuplicateChannelError(`Channel with URL ${newUrl} already exists`);
        }
      }
    }
  ...
  addType: (type) => {
    const data = readChannels();
    if (data[type]) {
      throw new Error(`Channel type '${type}' already exists`);
    }
    data[type] = [];
    writeChannels(data);
    return data;
  ...
  removeType: (type) => {
    const data = readChannels();
    if (!data[type]) {
      throw new Error(`Channel type '${type}' does not exist`);
    }
    if (type === 'everyone') {
      throw new Error("Cannot remove 'everyone' type");
    }
    delete data[type];
    writeChannels(data);
    return data;
  ...
  renameType: (oldType, newType) => {
    const data = readChannels();
    if (!data[oldType]) {
      throw new Error(`Channel type '${oldType}' does not exist`);
    }
    if (data[newType]) {
      throw new Error(`Channel type '${newType}' already exists`);
    }
    if (oldType === 'everyone') {
      throw new Error("Cannot rename 'everyone' type");
    }

    data[newType] = data[oldType];
    delete data[oldType];
    writeChannels(data);
    return data;
  },
