...
const ImportModal = ({ isOpen, onClose, onImportSuccess, currentChannels }) => {
                            ...
                            <>
                                ...
                                <div className="space-y-2 max-h-[40vh] overflow-y-auto custom-scrollbar pr-2 mb-6">
                                    {Object.keys(parsedData).map(category => {
                                        const total = Array.isArray(parsedData[category]) ? parsedData[category].length : 0;
                                        // Calculate stats
                                        const currentCatChannels = currentChannels && currentChannels[category] ? currentChannels[category] : [];
                                        const newChannels = parsedData[category] || [];
                                        ...
                                        newChannels.forEach(nc => {
                                            const url = typeof nc === 'string' ? nc : nc.url;
                                            const name = typeof nc === 'string' ? "Unnamed Channel" : nc.name;
                                            const existing = currentCatChannels.find(c => c.url === url);

                                            if (!existing) {
                                                newCount++;
                                            } else if (name && name !== "Unnamed Channel" && existing.name !== name) {
                                                updatedCount++;
                                            } else {
                                                unchangedCount++;
                                            }
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`flex flex-col p-3 rounded-lg cursor-pointer transition-colors ${selectedCategories[category] ? 'bg-secondary/20 border border-secondary/30' : 'bg-white/5 border border-white/10 hover:bg-white/10'
                                                    }`}
                                            >
                                                ...
                                                {/* Stats Bar */}
                                                {selectedCategories[category] && (
                                                    <div className="flex gap-1 text-[10px] font-mono uppercase tracking-wider pl-8">
                                                        {newCount > 0 && <span className="text-green-400">{newCount} New</span>}
                                                        {updatedCount > 0 && <span className="text-blue-400">{updatedCount} Upd</span>}
                                                        {unchangedCount > 0 && <span className="text-gray-500">{unchangedCount} Same</span>}
                                                    </div>
                                                )}
                                            </div>
                                        )
